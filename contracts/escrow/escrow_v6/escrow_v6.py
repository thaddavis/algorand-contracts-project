...
def approval():
    ...
    @Subroutine(TealType.none)
    def has_escrow_payment_1():
        return If(
            Balance(Global.current_application_address()
                    ) >= App.globalGet(GLOBAL_ESCROW_PAYMENT_1)
        ).Then(
            Approve()
        ).Else(
            Reject()
            ...
            Cond(
                ...
                [
                    And(
                        ...
                        Txn.application_args[0] == CONST  # DoSwap
                        # ASA1 Contract Address
                        Txn.application_args[1] == CONST
                        # ASA2 Contract Address
                        Txn.application_args[2] == CONST
                        # POOL1 Contract Address
                        Txn.application_args[3] == CONST
                        # POOL2 Contract Address
                        Txn.application_args[4] == CONST
                    ),
                    fund_contract()
                ],
                [
                    And(
                        ...
                        Txn.application_args[0] == HAS_ESCROW_PAYMENT_1
                    ),
