...
def test_app_info():
    ...
    assert res['params']['creator'] == config.contract_creator  # type: ignore


def test_contract_address_account_info():
    res = algod_client.account_info(config.application_account)  # type: ignore
    assert res['amount'] == 0
    assert res['min-balance'] == 100000


# test_contract_address_account_info()
